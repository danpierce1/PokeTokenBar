...
2. 🐣 **孵化。** [PokéAPI](https://pokeapi.co/) の**第1世代（全国図鑑 #1〜#151）**の進化系統から、公式の捕獲率で重み付けされて生まれます — よくいるポケモンほど出やすく、レア度で出現率が変わります。孵化したポケモンは育成中もすぐに **図鑑** に表示されます。孵化ごとに25種類のせいかくがひとつ決まり — **ごくまれな偶然で ✨ 色違いが生まれます**。
3. ⚡ **進化。** コーディングを続けると実際の進化ツリー（1/2/3段階、分岐）に沿って育ち、各段階で小さな演出が流れます。
4. 🎓 **卒業 & 収集。** 最終進化 + 閾値で **図鑑** に永久保存されます — レアなほど時間がかかり（ヘビーユーザーで common ≈0.3日 → legendary ≈2.4日）— 新しいタマゴが届きます。
...
<td width="55%" valign="middle">
<h3>メニューバーの相棒</h3>
動く Gen-I スプライトが今日のトークン合計（compact、例：<code>200.7M</code>）の隣に住んでいます。今日のコスト（<code>$</code>）や公式上限 <code>%</code> を追加しても、すべてオフにしてキャラクターだけにしても。
</td>
