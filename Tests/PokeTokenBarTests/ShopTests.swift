...
final class ShopTests: XCTestCase {
    ...
    func testMultipleBuysUntilBroke() {
        let remaining = RareCandy.price - 1
        let s = store(used: 2 * RareCandy.price + remaining) // 2개까지 가능, 3번째 실패
        XCTAssertTrue(s.buyRareCandy())
        XCTAssertTrue(s.buyRareCandy())
        XCTAssertFalse(s.buyRareCandy())
        XCTAssertEqual(s.rareCandyCount, 2)
        XCTAssertEqual(s.state.spentTokens, 2 * RareCandy.price)
        XCTAssertEqual(s.availableTokens, remaining)
    }
    ...
    /// 상점 목록은 가격 오름차순(민트 10M < 사탕 50M < 이로치 부적 300M).
    ...
    /// (회귀: 알이 ForEach 밖에서 무조건 맨 아래로 append 돼 300M 부적보다 아래에 놓이던 표시.)
    /// 등급 알을 인접 그룹으로 묶지 **않는** 것이 의도다 — 그러면 400M 희귀 알이 300M 부적 위로 올라가
